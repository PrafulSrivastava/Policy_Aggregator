"""
Germany Auswärtiges Amt (Foreign Office) Work visa policy fetcher.

This fetcher monitors the official German Foreign Office website for work visa information.
...
def fetch(url: str, metadata: Dict[str, Any]) -> FetchResult:
    """
    Fetch work visa policy content from Germany Auswärtiges Amt website.
    ...
    if result.success:
        ...
        logger.info(f"Successfully fetched work visa content from Auswärtiges Amt {url} ({len(result.raw_text)} chars)")
    else:
        logger.error(f"Failed to fetch work visa content from Auswärtiges Amt {url}: {result.error_message}")
