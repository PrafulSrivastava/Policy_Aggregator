...
- Created dashboard service (`api/services/dashboard.py`) that aggregates statistics from repositories
- Dashboard API endpoint (`GET /api/dashboard`) returns JSON with all dashboard statistics
- Dashboard template displays stats cards, recent changes table, and source health table
- Error highlighting: Sources with 3+ consecutive failures or recent errors (within 24 hours) shown in red
- Stale highlighting: Sources not checked beyond expected frequency + 1 day shown in yellow
- Performance optimizations: Used count queries instead of loading all records, optimized recent changes query with JOIN
- JavaScript added for row clickability on recent changes table
- All data fetched via service layer (not direct database access from UI)

### File List
**New Files:**
- `api/services/dashboard.py` - Dashboard statistics service
- `admin-ui/static/js/main.js` - JavaScript for dashboard interactivity

**Modified Files:**
- `api/routes/api.py` - Added dashboard router and `GET /api/dashboard` endpoint
- `api/routes/web.py` - Updated dashboard route to fetch and render statistics
- `api/main.py` - Registered dashboard router
- `admin-ui/templates/pages/dashboard.html` - Complete dashboard template with stats, changes, and health sections
- `admin-ui/templates/base.html` - Added JavaScript file reference
